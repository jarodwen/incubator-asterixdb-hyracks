...
public class HybridHashGrouper extends AbstractHistogramPushBasedGrouper {
    ...
    /**
     * A flag to indicate whether the input data has been globally partitioned (so
     * that the resident partition can be directly flushed as the final result)
     */
    ...
    public HybridHashGrouper(IHyracksTaskContext ctx, int[] keyFields, int[] decorFields, int framesLimit,
            ...
            int partitions, boolean useBloomFilter, boolean isInputGlobalHashPartitioned) throws HyracksDataException {
            ...
            for (int i = 0; i < tupleCount; i++) {
                ...
                if (!outputAppender.append(outputTupleBuilder.getFieldEndOffsets(), outputTupleBuilder.getByteArray(),
                    ...
                    if (partitionIndex >= 0) {
                        ...
                        if (!isInputGlobalHashPartitioned) {
                            profileIOOutNetwork++;
                        }
                    }
    ...
    public void wrapup() throws HyracksDataException {
        ...
        // flush the resident partition so that the fully aggregated results will not be processed again
        if (isInputGlobalHashPartitioned) {
            flushResidentAsFinalResult();
